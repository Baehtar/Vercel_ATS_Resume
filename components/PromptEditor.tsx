...
const editablePromptKeys: PromptKey[] = ["experience", "summary", "story", "interview_prep"];
  ...
  return (
    <div>
      ...
      <p className="caption" style={{ margin: "0 0 16px" }}>
        Edit the instructions sent to the AI. Experience, story, and interview prep prompts should keep their JSON output format; the summary prompt may return plain paragraph text or JSON with a summary key. Available placeholders: <code>{"{{discipline}}"}</code>, <code>{"{{label}}"}</code>, <code>{"{{adjective}}"}</code>, <code>{"{{storyDiscipline}}"}</code>, and <code>{"{{focusTech}}"}</code>.
      </p>
